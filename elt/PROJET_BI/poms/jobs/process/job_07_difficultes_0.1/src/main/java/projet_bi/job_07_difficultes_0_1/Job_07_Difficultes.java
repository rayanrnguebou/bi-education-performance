...
package projet_bi.job_07_difficultes_0_1;
...
/**
 * Job: Job_07_Difficultes Purpose: Job_07_Difficultes<br>
 * Description: Job_07_Difficultes <br>
 * 
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status
 */
public class Job_07_Difficultes implements TalendJob {
	...
	private final String jobName = "Job_07_Difficultes";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Job_07_Difficultes.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Job_07_Difficultes.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class difficulte_OutputStruct implements routines.system.IPersistableRow<difficulte_OutputStruct> {
		final static byte[] commonByteArrayLock_PROJET_BI_Job_07_Difficultes = new byte[0];
		static byte[] commonByteArray_PROJET_BI_Job_07_Difficultes = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PROJET_BI_Job_07_Difficultes.length) {
					if (length < 1024 && commonByteArray_PROJET_BI_Job_07_Difficultes.length == 0) {
						commonByteArray_PROJET_BI_Job_07_Difficultes = new byte[1024];
					} else {
						commonByteArray_PROJET_BI_Job_07_Difficultes = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PROJET_BI_Job_07_Difficultes, 0, length);
				strReturn = new String(commonByteArray_PROJET_BI_Job_07_Difficultes, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PROJET_BI_Job_07_Difficultes, 0, length);
				strReturn = new String(commonByteArray_PROJET_BI_Job_07_Difficultes, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PROJET_BI_Job_07_Difficultes) {
		 ...
		 * Compare keys
		 */
		public int compareTo(difficulte_OutputStruct other) {
	...
	public void tFileInputDelimited_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				difficulte_OutputStruct difficulte_Output = new difficulte_OutputStruct();
				...
				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "difficulte_Output");
				}
				...
				String tableName_tDBOutput_1 = "dim_difficultes_academiques";
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:mupuQhg/nZZ0GbU6HOIFZO4sabAQ6nPsaO2CkA==");
				...
				while (rsTable_tDBOutput_1.next()) {
					...
					if (table_tDBOutput_1.equalsIgnoreCase("dim_difficultes_academiques")) {
				...
				if (!whetherExist_tDBOutput_1) {
					try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
						stmtCreate_tDBOutput_1.execute("CREATE TABLE `" + tableName_tDBOutput_1
								+ "`(`id_difficulte` INT(2)   not null ,`id_etudiant` INT(2)  ,`maitrise_metier_etudiant` VARCHAR(14)  ,`difficulte_adaptation_methodes` VARCHAR(9)  ,`difficulte_taille_groupes` INT(1)  ,`interet_lecture_ecriture` VARCHAR(7)  ,`frequence_etude_lecons` VARCHAR(15)  ,`attente_veille_examens` INT(1)  ,`participation_examens_semestriels` VARCHAR(12)  ,`passe_scolaire_difficile` INT(1)  ,`decrochage_anterieur_sec_prim` INT(1)  ,`difficultes_pedagogiques_specifiques` VARCHAR(24)  )");
					}
				}
				String replace_tDBOutput_1 = "REPLACE INTO `" + "dim_difficultes_academiques"
						+ "` (`id_difficulte`,`id_etudiant`,`maitrise_metier_etudiant`,`difficulte_adaptation_methodes`,`difficulte_taille_groupes`,`interet_lecture_ecriture`,`frequence_etude_lecons`,`attente_veille_examens`,`participation_examens_semestriels`,`passe_scolaire_difficile`,`decrochage_anterieur_sec_prim`,`difficultes_pedagogiques_specifiques`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
...
// # Outputs initialization
				difficulte_OutputStruct difficulte_Output_tmp = new difficulte_OutputStruct();
// ###############################
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									row1.genre = ParserUtils.parseTo_Character(temp);
									...
									rowstate_tFileInputDelimited_1.setException(new RuntimeException(String.format(
											"Couldn't parse value for column '%s' in '%s', value is '%s'. Details: %s",
											"genre", "row1", temp, ex_tFileInputDelimited_1),
											ex_tFileInputDelimited_1));
								}
...
// Start of branch "etudiant_Out"
							if (etudiant_Out != null) {
								...
								if (!rejectedInnerJoin_tMap_3) { // G_TM_M_020
									...
									difficulte_OutHashKey.interet_lecture_ecriture = etudiant_Out.interet_lecture_ecriture;
								...
								if (tHash_Lookup_difficulte_Out != null
									...
									// difficulte_Out.interet_lecture_ecriture = '" +
									// difficulte_OutHashKey.interet_lecture_ecriture + "'");
								} // G 071
								...
								{ // start of Var scope
									...
									difficulte_Output = null;

									if (!rejectedInnerJoin_tMap_3) {

// # Output table : 'difficulte_Output'
										difficulte_Output_tmp.id_difficulte = difficulte_Out.id_difficulte;
										difficulte_Output_tmp.id_etudiant = difficulte_Out.id_etudiant;
										difficulte_Output_tmp.maitrise_metier_etudiant = (difficulte_Out.maitrise_metier_etudiant == null)
												? "Moyenne"
												: difficulte_Out.maitrise_metier_etudiant;
										difficulte_Output_tmp.difficulte_adaptation_methodes = java.util.Arrays
												.asList("Aucune", "Légère", "Modérée", "Sévère")
												.contains(difficulte_Out.difficulte_adaptation_methodes)
														? difficulte_Out.difficulte_adaptation_methodes
														: "Légère";
										difficulte_Output_tmp.difficulte_taille_groupes = (difficulte_Out.difficulte_taille_groupes == null)
												? 0
												: (difficulte_Out.difficulte_taille_groupes > 0 ? 1 : 0);
										difficulte_Output_tmp.interet_lecture_ecriture = (difficulte_Out.interet_lecture_ecriture == null)
												? "Moyen"
												: difficulte_Out.interet_lecture_ecriture;
										difficulte_Output_tmp.frequence_etude_lecons = java.util.Arrays.asList("Jamais",
												"Rarement", "Parfois", "Régulièrement", "Intensivement")
												.contains(difficulte_Out.frequence_etude_lecons)
														? difficulte_Out.frequence_etude_lecons
														: "Parfois";
										difficulte_Output_tmp.attente_veille_examens = ("Intensivement"
												.equals(difficulte_Out.frequence_etude_lecons)
												&& difficulte_Out.attente_veille_examens == 1) ? 0
														: (difficulte_Out.attente_veille_examens == null ? 0
																: (difficulte_Out.attente_veille_examens > 0 ? 1 : 0));
										difficulte_Output_tmp.participation_examens_semestriels = (difficulte_Out.participation_examens_semestriels == null)
												? "Tous"
												: difficulte_Out.participation_examens_semestriels;
										difficulte_Output_tmp.passe_scolaire_difficile = (difficulte_Out.passe_scolaire_difficile == null)
												? 0
												: (difficulte_Out.passe_scolaire_difficile > 0 ? 1 : 0);
										difficulte_Output_tmp.decrochage_anterieur_sec_prim = (difficulte_Out.decrochage_anterieur_sec_prim == null)
												? 0
												: (difficulte_Out.decrochage_anterieur_sec_prim > 0 ? 1 : 0);
										difficulte_Output_tmp.difficultes_pedagogiques_specifiques = (difficulte_Out.difficultes_pedagogiques_specifiques == null)
												? "Aucune"
												: difficulte_Out.difficultes_pedagogiques_specifiques;
										difficulte_Output = difficulte_Output_tmp;
...
// ###############################
								 ...
								 */
// Start of branch "difficulte_Output"
								if (difficulte_Output != null) {
									...
									pstmt_tDBOutput_1.setInt(1, difficulte_Output.id_difficulte);

									if (difficulte_Output.id_etudiant == null) {
										pstmt_tDBOutput_1.setNull(2, java.sql.Types.INTEGER);
									} else {
										pstmt_tDBOutput_1.setInt(2, difficulte_Output.id_etudiant);
									}

									if (difficulte_Output.maitrise_metier_etudiant == null) {
										pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(3, difficulte_Output.maitrise_metier_etudiant);
									}

									if (difficulte_Output.difficulte_adaptation_methodes == null) {
										pstmt_tDBOutput_1.setNull(4, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(4,
												difficulte_Output.difficulte_adaptation_methodes);
									}

									if (difficulte_Output.difficulte_taille_groupes == null) {
										pstmt_tDBOutput_1.setNull(5, java.sql.Types.INTEGER);
									} else {
										pstmt_tDBOutput_1.setInt(5, difficulte_Output.difficulte_taille_groupes);
									}

									if (difficulte_Output.interet_lecture_ecriture == null) {
										pstmt_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(6, difficulte_Output.interet_lecture_ecriture);
									}

									if (difficulte_Output.frequence_etude_lecons == null) {
										pstmt_tDBOutput_1.setNull(7, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(7, difficulte_Output.frequence_etude_lecons);
									}

									if (difficulte_Output.attente_veille_examens == null) {
										pstmt_tDBOutput_1.setNull(8, java.sql.Types.INTEGER);
									} else {
										pstmt_tDBOutput_1.setInt(8, difficulte_Output.attente_veille_examens);
									}

									if (difficulte_Output.participation_examens_semestriels == null) {
										pstmt_tDBOutput_1.setNull(9, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(9,
												difficulte_Output.participation_examens_semestriels);
									}

									if (difficulte_Output.passe_scolaire_difficile == null) {
										pstmt_tDBOutput_1.setNull(10, java.sql.Types.INTEGER);
									} else {
										pstmt_tDBOutput_1.setInt(10, difficulte_Output.passe_scolaire_difficile);
									}

									if (difficulte_Output.decrochage_anterieur_sec_prim == null) {
										pstmt_tDBOutput_1.setNull(11, java.sql.Types.INTEGER);
									} else {
										pstmt_tDBOutput_1.setInt(11, difficulte_Output.decrochage_anterieur_sec_prim);
									}

									if (difficulte_Output.difficultes_pedagogiques_specifiques == null) {
										pstmt_tDBOutput_1.setNull(12, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(12,
												difficulte_Output.difficultes_pedagogiques_specifiques);
									}
								...
								} // End of branch "difficulte_Output"
...
// ###############################      
				...
				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "difficulte_Output");
				}
	...
	public static class difficulte_OutStruct
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime * result
						+ ((this.interet_lecture_ecriture == null) ? 0 : this.interet_lecture_ecriture.hashCode());
		...
		public boolean equals(Object obj) {
			...
			if (this.interet_lecture_ecriture == null) {
				if (other.interet_lecture_ecriture != null)
					return false;

			} else if (!this.interet_lecture_ecriture.equals(other.interet_lecture_ecriture))
		...
		public int compareTo(difficulte_OutStruct other) {
			...
			returnValue = checkNullsAndCompare(this.interet_lecture_ecriture, other.interet_lecture_ecriture);
			if (returnValue != 0) {
				return returnValue;
			}
	...
	public void tFileInputDelimited_2Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// linked node: tMap_3 - inputs:(etudiant_Out,difficulte_Out)
				// outputs:(difficulte_Output)
...
// ###############################      
	...
	public static void main(String[] args) {
		final Job_07_Difficultes Job_07_DifficultesClass = new Job_07_Difficultes();

		int exitCode = Job_07_DifficultesClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Job_07_Difficultes.class.getClassLoader()
					.getResourceAsStream("projet_bi/job_07_difficultes_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Job_07_Difficultes.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : Job_07_Difficultes");
		}
...
/************************************************************************************************
 * 184627 characters generated by Talend Open Studio for Data Integration on the
 * 10 mai 2026 à 23:12:27 WAT
 ************************************************************************************************/